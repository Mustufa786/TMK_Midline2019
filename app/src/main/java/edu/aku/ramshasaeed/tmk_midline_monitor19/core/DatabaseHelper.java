package edu.aku.ramshasaeed.tmk_midline_monitor19.core;
...
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.AreasContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.AreasContract.singleAreas;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.BLRandomContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.BLRandomContract.singleChild;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.FamilyMembersContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.FamilyMembersContract.familyMembers;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.FormsContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.FormsContract.FormsTable;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.SectionIIMContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.SectionIIMContract.singleIm;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.TalukasContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.TalukasContract.singleTalukas;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.UCsContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.UCsContract.singleUCs;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.UsersContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.UsersContract.singleUser;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.VersionAppContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.VersionAppContract.VersionAppTable;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.VillagesContract;
import edu.aku.ramshasaeed.tmk_midline_monitor19.contracts.VillagesContract.singleVillages;
...
//                + singleVillages.COLUMN_TALUKA + " TEXT,"
    ...
    private static final String SQL_ALTER_FAMILYMEMBER_RENAME = "ALTER TABLE " +
            familyMembers.TABLE_NAME + " RENAME TO " + familyMembers.TABLE_NAME + "_old;";
    private static final String SQL_ALTER_FORMS_ADD_CLUSTER = "ALTER TABLE " +
            FormsTable.TABLE_NAME + " ADD COLUMN " +
            FormsTable.COLUMN_CLUSTER + " TEXT ";
    private static final String SQL_ALTER_FORMS_ADD_APPVERSION = "ALTER TABLE " +
            FormsTable.TABLE_NAME + " ADD COLUMN " +
            FormsTable.COLUMN_APPVER + " TEXT ";
      ...
      /*  db.execSQL(SQL_DELETE_VILLAGES);
        ...
        /*switch (i) {
            case 1:
                ...
                db.execSQL(SQL_ALTER_FORMS_ADD_APPVERSION);
...
//                singleVillages.COLUMN_TALUKA,
    ...
    public void syncMembers(JSONArray syncList) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(familyMembers.TABLE_NAME_GET, null, null);
        try {
            JSONArray jsonArray = syncList;
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject jsonObjectCC = jsonArray.getJSONObject(i);

                FamilyMembersContract fmc = new FamilyMembersContract();
                fmc.Sync(jsonObjectCC);
                ...
                values.put(familyMembers.COLUMN_MMNAME, fmc.getMmname());
                ...
                db.insert(familyMembers.TABLE_NAME_GET, null, values);
            }
        } catch (Exception e) {
        } finally {
            db.close();
...
// Which row to update, based on the ID
    ...
    public Collection<FamilyMembersContract> getUnder5Children(String subAreaCode, String hh) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor c = null;
        String[] columns = {
                familyMembers.COLUMN_ID,
        ...
        try {
            c = db.query(
                    familyMembers.TABLE_NAME_GET,  // The table to query
                    columns,                   // The columns to return
                    whereClause,               // The columns for the WHERE clause
                    whereArgs,                 // The values for the WHERE clause
                    groupBy,                   // don't group the rows
            ...
            while (c.moveToNext()) {
                allCC.add(new FamilyMembersContract().Hydrate(c, true));
            }
        } finally {
            if (c != null) {
                c.close();
